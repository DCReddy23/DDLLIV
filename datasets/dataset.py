...
from datasets.data_augment import (
    PairCompose, PairToTensor, PairRandomHorizontalFilp,
    PairRandomVerticalFlip, PairRandomRotation, PairColorJitter
)


class LLdataset:
    """Low-Light dataset wrapper that provides train and validation loaders.

    Args:
        config: Config namespace with data.*, training.*, and sampling.* fields.
    """
    ...
    def get_loaders(self):
        ...
        train_loader = torch.utils.data.DataLoader(
            ...
            pin_memory=True,
            drop_last=True,           # avoid batch size mismatch at epoch end
...
class AllWeatherDataset(torch.utils.data.Dataset):
    """Paired low-light / normal-light image dataset.

    Reads image pairs from a text file where each line contains:
        <low_light_path> <normal_light_path>

    Args:
        dir: Root data directory.
        patch_size: Size for random cropping (used only during training).
        filelist: Name of the text file listing image pairs.
        train: Whether this is the training set. Default: True.
    """
    ...
    def __init__(self, dir, patch_size, filelist=None, train=True):
        ...
        if train:
            ...
            # 4. Mild color jitter on input only (robustness)
            # 5. Convert to tensor
            self.transforms = PairCompose([
                PairRandomHorizontalFilp(),
                PairRandomVerticalFlip(),
                PairRandomRotation(),
                PairColorJitter(brightness=0.05, contrast=0.05),
                PairToTensor()
            ])
        else:
            self.transforms = PairCompose([
